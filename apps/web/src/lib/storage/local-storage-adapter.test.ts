...
describe('localStorageAdapter', () => {
  ...
  it('backfills missing pov_character_id on legacy scenes', () => {
    ...
    expect(scene.arc_events).toEqual([])
  ...
  it('upserts and deletes characters', () => {
    const project = localStorageAdapter.fetchProjects()[0]
    const baseChar = { role: 'Protagonist', color: '#7c3aed', summary: 'Main character', age: '', pronouns: 'she/her', relationships: '', traits: ['brave'], arc: null }
  ...
  it('reorders scenes by ordered id list', () => {
    ...
    const sceneInput = {
      summary: '',
      location_id: null,
      arc_events: [] as { character_id: string; beat_id: string | null; note: string; sort_order: number }[],
      word_count: 0,
